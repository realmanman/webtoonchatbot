...
def build_or_load_vectorstores():
    ...
    retriever_law = vectordb.as_retriever(search_kwargs={"k": 3})
...
#     rag_chain = RetrievalQAWithSourcesChain.from_chain_type(llm=llm, retriever=retriever)
...
#     return df, rag_chain, vectordb, retriever_info, memory_info

# df, rag_chain, vectordb, retriever_info, memory_info = load_components()

# # Prompt template for '정보' intent
# prompt_template_info = ChatPromptTemplate.from_messages([
#     ("system", "너는 웹툰 전문가야. 반드시 웹툰 데이터베이스에 있는 내용만으로 답변해야 해. \
#     검색된 문서 외에는 그 어떤 정보도 제공하면 안 돼. 문서가 없으면 '해당 정보를 찾을 수 없습니다'라고만 말해."),
#     ("human", "질문: {question}\n\n관련 문서:\n{context}")
# ])
...
# def classify_intent(question: str) -> str:
#     prompt = f"""사용자 질문을 보고 intent를 '법률', '정보', '현황', '추천' 중 하나로 분류해줘.
    
# - '법률'은 웹툰을 드라마,영화,게임,애니메이션 등으로 2차 창작할 때 법률, 법령, 저작권, 계약과 관련된 법적 사안을 묻는 질문이야.
# - '정보': 제목, 줄거리, 작가, 등장인물 등 웹툰 자체 정보
# - '추천'은 웹툰을 드라마,영화,게임,애니메이션 등으로 2차 창작할 때 적합한 웹툰을 추천해달라는 질문이야. 2차 창작물(드라마, 영화, 게임, 애니메이션 등)에 대한 얘기가 없으면 추천이 아니야.
# - '현황'은 웹툰의 조회수, 구독자수, 평점, 순위, 연령/성별 선호도 등 웹툰의 현재 상태나 통계, 순위를 알려달라는 질문이야.
...
# def extract_top_k(question: str, default_k=3) -> int:
#     match = re.search(r"(\d+)[개명편]", question)
#     if match:
#         return int(match.group(1))
#     return default_k

# def extract_filter_conditions(question: str) -> dict:
#     prompt = f"""다음 질문에서 웹툰 추천/현황 분석에 필요한 조건을 추출해줘.
# - 카테고리: 장르 정보 (예: 로맨스, 무협, 드라마 등)
# - 연령층: 10~20대, 30대, 40대 중 택1
# - 성별: 남성, 여성 중 택1
...
#     if "카테고리" in conditions and conditions["카테고리"]:
#         filtered = filtered[filtered["카테고리"].str.contains(conditions["카테고리"], case=False, na=False)]
#     if "연령층" in conditions and conditions["연령층"] in df.columns:
#         filtered = filtered[filtered[conditions["연령층"]].str.contains("상|중", na=False)]
#     if "성별" in conditions and conditions["성별"]:
#         filtered = filtered[filtered["성별선호도"].str.contains(conditions["성별"], case=False, na=False)]
#     if filtered.empty:
#         return "❗ 조건에 맞는 웹툰을 찾지 못했습니다."

#     filtered["추천점수"] = filtered["평점"] * 1 + (filtered["구독자수"] / 1000)
#     top = filtered.sort_values("추천점수", ascending=False).head(k)

#     return "\n".join(
#         f"- {row['제목']} (평점: {row['평점']}, 구독자수: {int(row['구독자수'])}, 키워드: {row['키워드']})"
...
#     if "구독자수" in question:
#         top = filtered.sort_values("구독자수", ascending=False).head(k)
#     elif "평점" in question:
#         top = filtered.sort_values("평점", ascending=False).head(k)
#     else:
#         top = filtered.sort_values(["평점", "구독자수"], ascending=False).head(k)

#     if top.empty:
#         return "❗ 조건에 맞는 웹툰이 없습니다."

#     return "\n".join(
#         f"- {row['제목']} (평점: {row['평점']}, 구독자수: {int(row['구독자수'])}, 카테고리: {row['카테고리']})"
#         for _, row in top.iterrows()
#     )
